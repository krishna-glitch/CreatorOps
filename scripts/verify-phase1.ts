...
async function verify() {
    ...
    try {
      ...
      ORDER BY table_name
    `;
        const tableNames = tableCheck.map((r: Record<string, string>) => r.table_name);
    ...
    `;
        const brandColNames = brandCols.map((r: Record<string, string>) => r.column_name);
        const expectedBrandCols = ['id', 'user_id', 'name', 'created_at', 'updated_at'];
        const brandMatch = expectedBrandCols.every(c => brandColNames.includes(c));
        if (brandMatch) {
      ...
      ORDER BY ordinal_position
    `;
        const dealColNames = dealCols.map((r: Record<string, string>) => r.column_name);
        const expectedDealCols = ['id', 'user_id', 'brand_id', 'title', 'total_value', 'currency', 'status', 'created_at', 'updated_at'];
    ...
    `;
        for (const fk of fks) {
            pass(`${fk.table_name}.${fk.column_name} → ${fk.foreign_table_schema}.${fk.foreign_table_name}.${fk.foreign_column_name}`);
        }
    ...
    `;
        ...
        const expectedIndexes = ['brands_user_id_idx', 'deals_user_id_idx', 'deals_brand_id_idx', 'deals_status_idx'];
        for (const expected of expectedIndexes) {
            if (!indexes.some((i: Record<string, string>) => i.indexname === expected)) {
    ...
    try {
        ...
        // Check deal statuses
        const dealStatuses = await db.select({ status: deals.status, title: deals.title }).from(deals);
