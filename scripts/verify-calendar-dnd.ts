...
async function main() {
  ...
  if (deliverableRows.length === 0) {
    console.log(
      "⚠️ No suitable deliverable found to test. creating one or skipping.",
    );
  } else {
    const deliverable = deliverableRows[0] as {
      id: string;
    ...
    try {
      ...
      if (Math.abs(updatedDate.getTime() - newDate.getTime()) < 1000) {
        ...
        console.error(
          `❌ Persistence failed. Got ${updatedDate.toISOString()}, expected ${newDate.toISOString()}`,
        );
      }
