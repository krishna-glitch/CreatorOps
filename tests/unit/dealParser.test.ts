...
describe("extractAmount", () => {
    ...
    it("uses last amount in multi-currency negotiation", () => {
        expect(
            extractAmount("Initial ask $1500, final agreed ₹50,000"),
        ).toBe(50000);
    });
...
describe("extractBrandCandidate", () => {
    ...
    it("matches case-insensitively", () => {
        expect(extractBrandCandidate("ADIDAS wants a post", brands)).toBe(
            "Adidas",
        );
    });
    ...
    it("infers brand from explicit label", () => {
        expect(
            extractBrandCandidate("Brand: Acme Co wants 2 reels", []),
        ).toBe("Acme Co");
    });
