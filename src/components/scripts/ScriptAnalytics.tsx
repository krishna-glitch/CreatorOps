...
function getReadabilityTip(score: number): string {
  ...
  if (score >= 50)
    return "Good baseline. Try shorter sentences to improve clarity.";
...
function getEngagementTip(metrics: ScriptMetrics): string {
  if (!metrics.engagement.factors.hasQuestions)
    return "Add a question to boost engagement.";
  if (!metrics.engagement.factors.hasCallToAction)
    return "Add a clear CTA like “comment below” or “check link in bio.”";
  if (metrics.engagement.factors.personalPronouns < 2)
    return "Use “you/your” more often to make the script feel personal.";
...
function getKeywordTip(keywords: ScriptMetrics["keywords"]): string {
  if (keywords.length === 0)
    return "Add specific topical words to strengthen relevance.";
...
function _compareLabel(
  current: number,
  average: number,
  isSentiment = false,
): string {
  const rawDelta = current - average;
  const delta = isSentiment
...
function _ExportPdfButton({ onExport }: { onExport: () => void }) {
  return (
    <Button
      type="button"
      variant="outline"
...
export function ScriptAnalytics({
  ...
  const sentimentDetails = useMemo(
    () => getSentimentWordBreakdown(text),
    [text],
  );
  const _topAverages = {
    ...DEFAULT_TOP_PERFORMER_AVGS,
    ...topPerformerAverages,
  };
    ...
    <div className={cn("flex flex-col gap-4", className)}>
      <div className="rounded-xl border dash-border dash-bg-card p-4 shadow-sm">
        ...
        <div className="mt-4 space-y-3">
          ...
          >
            ...
            >
              <div className="flex items-end justify-between">
                ...
                <Badge
                  variant="outline"
                  className={cn("mb-1", qualityTone.badgeClass)}
                >
                  {qualityTone.label}
                </Badge>
              </div>
          ...
          <Section
            title="Readability"
            expanded={expanded.readability}
            onToggle={() => toggle("readability")}
          >
            <div className="space-y-3">
              ...
              <div className="rounded-md bg-emerald-50/50 px-3 py-2 text-xs text-emerald-800 border border-emerald-100">
                ...
                {getReadabilityTip(metrics.readability.score)}
              </div>
            </div>
          </Section>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div
                  className={cn(
                    ...
                    metrics.engagement.factors.hasQuestions
                      ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                      : "dash-border dash-bg-card text-slate-500",
                  )}
                >
                  {metrics.engagement.factors.hasQuestions ? (
                  ...
                  <span>
                    Questions ({metrics.engagement.factors.questionCount})
                  </span>
                </div>
                <div
                  className={cn(
                    "flex items-center gap-2 rounded border p-2 text-xs",
                    metrics.engagement.factors.hasCallToAction
                      ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                      : "dash-border dash-bg-card text-slate-500",
                  )}
                >
                ...
                <div className="flex items-center justify-between rounded border dash-border dash-bg-card p-2 text-xs text-slate-600">
                  ...
                  <span className="font-semibold">
                    {metrics.engagement.factors.emojiCount}
                  </span>
                </div>
                <div className="flex items-center justify-between rounded border dash-border dash-bg-card p-2 text-xs text-slate-600">
                  <span>"You/Your"</span>
                  <span className="font-semibold">
                    {metrics.engagement.factors.personalPronouns}
                  </span>
                </div>
              </div>
          ...
          >
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Badge
                  variant="outline"
                  className={cn(getSentimentBadge(metrics.sentiment.sentiment))}
                >
                  {metrics.sentiment.sentiment.toUpperCase()}
                </Badge>
                <span className="text-xs font-medium text-slate-500">
                  Intensity: {metrics.sentiment.score}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                  ...
                  {sentimentDetails.positive.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {sentimentDetails.positive.map((w) => (
                        <span
                          key={w}
                          className="rounded bg-emerald-50 px-1.5 py-0.5 text-[10px] text-emerald-700 border border-emerald-100"
                        >
                          {w}
                        </span>
                      ))}
                ...
                <div className="space-y-1">
                  <p className="text-[10px] uppercase text-rose-600 font-semibold mb-1">
                    Negatives
                  </p>
                  {sentimentDetails.negative.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {sentimentDetails.negative.map((w) => (
                        <span
                          key={w}
                          className="rounded bg-rose-50 px-1.5 py-0.5 text-[10px] text-rose-700 border border-rose-100"
                        >
                          {w}
                        </span>
                      ))}
          ...
          >
            <div className="space-y-3">
              ...
              <div className="rounded-md bg-blue-50/50 px-3 py-2 text-xs text-blue-800 border border-blue-100">
                <span className="font-medium">Tip:</span>{" "}
                {getKeywordTip(metrics.keywords)}
              </div>
            </div>
          </Section>
          ...
          <Section
            title="Quick Stats"
            expanded={expanded.quickStats}
            onToggle={() => toggle("quickStats")}
          >
            <div className="grid grid-cols-4 gap-2 text-center">
              {[
                ...
                <div
                  key={stat.label}
                  className={cn(
                    "rounded border border-slate-100 dash-bg-card p-2",
                    stat.colSpan && `col-span-${stat.colSpan}`,
                  )}
                >
